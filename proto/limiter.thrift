...
service Limiter {
    ...
    /**
     * Получение значений лимитов на момент времени запроса. Если сделать
     * два запроса, между которыми лимит был изменен, то вернет разные значения.
     */
    ...
    /**
     * Получение значений лимитов после их изменения в текущей операции. 
     * В момент совершения операции 'холд' значение лимита меняется, данный метод
     * будет возвращать это значение вне зависимости от последующих изменений лимита
     */
